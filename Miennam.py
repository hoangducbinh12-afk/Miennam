...
def get_mapping_82bit(full_str, total_pos=82):
    if not full_str or len(full_str) < total_pos:
        return {str(i): f"{i % 100:02d}" for i in range(total_pos * total_pos)}
...
def update_matrix_state(db, results_18, mapping):
    for wire_id, w_data in db.items():
        ...
        if num in results_18:
            ...
            w_data["score"] = w_data.get("score", 1000.0) - 1.8 # Tối ưu cho 18 lô
...
# --- 2. BỘ NÃO LỌC TẦNG GIAO THOA (OVERLAP EDITION) ---
def thermal_ai_engines_v14(df_raw, history, db, mapping, cfg):
    ...
    # BƯỚC 2: TÍNH GIAO THOA LAG T-2
    # Lấy Top 79 của T-1 (Dựa trên điểm nền Rank)
    top_79_t1 = set(df_raw.sort_values("Rank")["Số"].head(79))
    
    # Lấy Top 79 của T-2 (Dùng điểm Score thuần từ DB)
    top_79_t2 = set()
    if len(history) >= 2:
        t2_wires = sorted(db.items(), key=lambda x: x[1]['score'])[:3000] # Lấy cụm dây mạnh nhất
        top_79_t2 = {f"{int(mapping.get(w_id)):02d}" for w_id, d in t2_wires if mapping.get(w_id)}

    # BƯỚC 3: CỘNG ĐIỂM SÁT THỦ
    def apply_overlap_logic(row):
        s = row['Số']
        score = row['Điểm']
        # Thưởng Giao Thoa
        ...
        # Thưởng 88-Base
        if s in base_88: score += 500000
        return score

    df_raw['Final_Score'] = df_raw.apply(apply_overlap_logic, axis=1)
    df_sorted = df_raw.sort_values(by=['Final_Score', 'Số'], ascending=[False, True])
    
    dk_39 = df_sorted.head(39)
    da_59 = df_sorted.head(59)
    ds_79 = df_sorted.head(79)
    
    return dk_39, da_59, ds_79, df_sorted, (t2, b4), base_88

# --- 3. UI DASHBOARD ---
st.set_page_config(layout="wide", page_title="Matrix 18-Solo V14.2.5")
st.title("🛡️ Matrix 18-Solo V14.2.5 (82-Bit)")

if 'cfg' not in st.session_state: st.session_state['cfg'] = {"tier": 58, "win": 12, "bot": 40, "bet": 40}
...
with st.sidebar:
    ...
    st.header("📸 2. QUÉT KQ")
    up_img = st.file_uploader("Ảnh KQ (18 Lô)", type=['jpg', 'png', 'jpeg'])
    if up_img and st.button("🚀 CHẠY OCR THÔNG MINH"):
        ...
        if len(nums) >= 18:
            # Logic nhận diện hướng ảnh
            if len(nums[0]) < len(nums[-1]):
                st.warning("Ảnh ngược (G8 -> GĐB). Đang đảo lại...")
                nums = nums[::-1]
    ...
    if st.button("🔥 PHÂN TÍCH & LƯU", type="primary", use_container_width=True):
        ...
        if len(raw_list) >= 18 and gdb_val and st.session_state['db']:
    ...
    st.session_state['cfg']['tier'] = st.slider("Tầng Sạch (%):", 40, 80, 58) # Ngưỡng chuẩn 58%
...
if st.session_state['last_full_str']:
    ...
    m3.info(f"V14.2.5: 82-Bit | Tầng Sạch {st.session_state['cfg']['tier']}%")
    ...
    st.download_button("💾 XUẤT MASTER JSON", data=json.dumps({"matrix": st.session_state['db'], "history": st.session_state['history'], "last_full_str": st.session_state['last_full_str']}, ensure_ascii=False), file_name="matrix_18solo_v14.json", use_container_width=True)